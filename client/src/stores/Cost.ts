...
export const CostStore = defineStore({
  ...
  state: (): CostStore => {
    return {
      ...
      costs: [],
      categories: [
        { value: 'Food', icon: 'mdi mdi-food-outline', color: 'A8D9B' },
  ...
  getters: {
    ...
    categoriesTotal(state): {} {
      ...
      state.costs.map((cost) => {
        ...
        if (category in categorTotal)
          categorTotal[category] = Number(categorTotal[category]) + cost.price
  ...
  actions: {
    ...
    async deleteCost(id: string): Promise<void> {
      try {
        const token = await refreshTokenUser()
        const costs = await api.delete(`/cost/${id}`, {
